...
convert <- function(pattern_list1, pattern_list2, date1, date2, dates, metadata) {
  and_or <- NULL
  ...
  for(i in 1:length(pattern_list1)){
    ...
    query_array <- c(query_array, 6, time_step1[i], 0, pattern_number1[i], pattern_number2[j], and_or[i+j])
    }
  }
