...
luccpl_plot_input <- function(X, date, pattern, color, pathMPinput, map_title, Width, Height) {
    ...
    myKey <- list(rectangles = list(col = color), text = list(lab = pattern), space = "bottom", font = 0.6, width = 0.8, columns = 4)
    jpeg(pathMPinput, width = Width, height = Height)
    print(rasterVis::levelplot(X, main = list(map_title, side = 1, line = 0.5), col.regions = color, maxpixels = 1e+09, colorkey = FALSE, 
        names.attr = substr(date, 1, 4), key = myKey, scales = list(draw = FALSE), xscale.components = xscale.raster, yscale.components = yscale.raster, 
        pretty = TRUE))
    dev.off()
}
...
luccpl_plot_output <- function(X, date, pathMPoutput, map_title, Width, Height) {
    ...
    myKey <- list(rectangles = list(col = color1), text = list(lab = c("False", "", "True", "")), space = "bottom", font = 0.6, width = 0.8, 
        columns = 2)
    jpeg(pathMPoutput, width = Width, height = Height)
    print(rasterVis::levelplot(X, main = list(map_title, side = 1, line = 0.5), col.regions = color1, maxpixels = 1e+09, colorkey = FALSE, 
        names.attr = substr(date, 1, 4), key = myKey, scales = list(draw = FALSE), xscale.components = xscale.raster, yscale.components = yscale.raster, 
        pretty = TRUE))
    dev.off()
}
